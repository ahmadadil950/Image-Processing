...
GLWidget::allocateTextureFBO(int w, int h) 
{
	...
	if (status != GL_FRAMEBUFFER_COMPLETE) {
		qDebug() << "error in allocateTextureFBO";
	}
...
// GLWidget::
//
// Delete all allocated textures and frame buffers.
//
void
GLWidget::deleteBuffers()
{

	glDeleteTextures(1, &m_texture_fbo[PASS1]);
	glDeleteTextures(1, &m_texture_fbo[PASS2]);

	glDeleteFramebuffers (1, &m_fbo[PASS1]);
	glDeleteFramebuffers (1, &m_fbo[PASS2]);
...
GLWidget::initShader(QGLShaderProgram &program, QString vshaderName, QString fshaderName, UniformMap &uniformsMap, int *uniforms)
{
	...
	if(!program.addShaderFromSourceFile(QGLShader::Fragment, fshaderName)) {
		...
		msg = QMessageBox::critical(0, "Error", "Fragment shader error: " + fshaderName + "\n" +
					program.log(), QMessageBox::Ok);
...
GLWidget::initShaders()
{
	...
	g_mainWindowP->imageFilter(PERSPECTIVE )->initShader();
...
//
void
GLWidget::applyFilterGPU(int nPasses, int w, int h) 
{
