...
MainWindow::createActions()
{
	...
	m_actionPerspective = new QAction("&Perspective Warp", this);
	m_actionPerspective->setShortcut(tr("Ctrl+P"));
	m_actionPerspective->setData(PERSPECTIVE);

	m_actionTrapezoid = new QAction("&Trapezoid Warp", this);
	m_actionTrapezoid->setShortcut(tr("Ctrl+T"));
	m_actionTrapezoid->setData(TRAPEZOID);
...
MainWindow::createMenus()
{
	...
	m_menuGeoOps->addAction(m_actionPerspective);
	m_menuGeoOps->addAction(m_actionTrapezoid);
...
MainWindow::createGroupPanel()
{
	...
	m_imageFilter[PERSPECTIVE]= new Perspective;
	m_imageFilter[TRAPEZOID ]= new Trapezoid;
	...
	m_stackWidgetPanels->addWidget(m_imageFilter[PERSPECTIVE ]->controlPanel());
	m_stackWidgetPanels->addWidget(m_imageFilter[TRAPEZOID   ]->controlPanel());
...
MainWindow::preview()
{
	...
	if(m_code > 0) {
		m_imageFilter[m_code]->applyFilter(m_imageSrc, gpuFlag(), m_imageDst);
		if (!gpuFlag())// set the texture for input or output
			display(1);
		else // for gpu case does not need setting texture
			m_glw->updateGL();
	} else {
...
MainWindow::execute(QAction* action)
{
	...
	// these operations require additional inputs before applying the filter
	if (m_code == CONVOLVE || m_code == PERSPECTIVE)
